...
namespace Triggered.Components
{
    public partial class MessagesList : ComponentBase, IDisposable
    {
        ...
        [Inject]
        public TriggeredDbContext TriggeredDbContext { get; set; } = null!;
        ...
        {
            IsMuted = TriggeredDbContext.Settings.GetSetting("MessageNotificationsEnabled")
                .Equals("False", StringComparison.InvariantCultureIgnoreCase);

            Volume = double.TryParse(TriggeredDbContext.Settings.GetSetting("MessageNotificationVolume"), out double volume) ?
                Math.Min(Math.Max(volume, 0), 1) : 0.25;

            FilteredLogLevels = TriggeredDbContext.Settings.GetSetting("MessageLevels")
        ...
        {
            IsMuted = !IsMuted;
            TriggeredDbContext.Settings.SetSetting("MessageNotificationsEnabled", (!IsMuted).ToString());
        ...
        private async Task FilterMessages(ChangeEventArgs changeEventArgs)
        {
            ...
            {
                TriggeredDbContext.Settings.SetSetting("MessageLevels", string.Join(", ", selectedLogLevels));
